...
def telaAluno(request,ra):
    acertos = acertosErros.objects.filter(aluno=ra).values_list("acertos",flat=True).last()
    erros = acertosErros.objects.filter(aluno=ra).values_list("erros",flat=True).last()
    nivelDoAluno = acertosErros.objects.filter(aluno=ra).values_list("nivelDoAluno",flat=True).last()
...
def criarFormulario(request,idProfessor):
    ...
    if 'formulario' in request.POST:
            ...
            ano_letivo = turma.split("|")[-2]
            classe = turma.split("|")[-1]      
            turmas = Turmas.objects.raw("SELECT idTurma, ano_letivo, classe, alu_id,prof_id FROM turmas where prof_id=%s GROUP BY classe,ano_letivo",[idProfessor])
...
def graficoAluno(request,ra):
    ...
    if 'responderFomulario' in request.POST:
        ...
        respostasAluno = []
        pkAluno = Aluno(ra=ra)    
        classe = Turmas.objects.filter(alu=pkAluno).values_list("classe",flat=True).last()
        ano_letivo = Turmas.objects.filter(alu=pkAluno).values_list("ano_letivo",flat=True).last()
        questao1 = RespostasFormulario.objects.values_list("respostaQuestao1",flat=True).filter(classe=classe).filter(ano_letivo=ano_letivo).last()
        questao2 = RespostasFormulario.objects.values_list("respostaQuestao2",flat=True).filter(classe=classe).filter(ano_letivo=ano_letivo).last()
        questao3 = RespostasFormulario.objects.values_list("respostaQuestao3",flat=True).filter(classe=classe).filter(ano_letivo=ano_letivo).last()
        questao4 = RespostasFormulario.objects.values_list("respostaQuestao4",flat=True).filter(classe=classe).filter(ano_letivo=ano_letivo).last()
        questao5 = RespostasFormulario.objects.values_list("respostaQuestao5",flat=True).filter(classe=classe).filter(ano_letivo=ano_letivo).last()
        questao6 = RespostasFormulario.objects.values_list("respostaQuestao6",flat=True).filter(classe=classe).filter(ano_letivo=ano_letivo).last()
        questao7 = RespostasFormulario.objects.values_list("respostaQuestao7",flat=True).filter(classe=classe).filter(ano_letivo=ano_letivo).last()
        questao8 = RespostasFormulario.objects.values_list("respostaQuestao8",flat=True).filter(classe=classe).filter(ano_letivo=ano_letivo).last()
        questao9 = RespostasFormulario.objects.values_list("respostaQuestao9",flat=True).filter(classe=classe).filter(ano_letivo=ano_letivo).last()
        questao10 = RespostasFormulario.objects.values_list("respostaQuestao10",flat=True).filter(classe=classe).filter(ano_letivo=ano_letivo).last()
        ...
        classe = Turmas.objects.filter(alu=pkAluno).values_list("classe",flat=True).last()
        ano_letivo = Turmas.objects.filter(alu=pkAluno).values_list("ano_letivo",flat=True).last()
        resposta1 = questoesEscolhidasAluno.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao1",flat=True).last()
        resposta2 = questoesEscolhidasAluno.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao2",flat=True).last()
        resposta3 = questoesEscolhidasAluno.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao3",flat=True).last()
        resposta4 = questoesEscolhidasAluno.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao4",flat=True).last()
        resposta5 = questoesEscolhidasAluno.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao5",flat=True).last()
        resposta6 = questoesEscolhidasAluno.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao6",flat=True).last()
        resposta7 = questoesEscolhidasAluno.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao7",flat=True).last()
        resposta8 = questoesEscolhidasAluno.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao8",flat=True).last()
        resposta9 = questoesEscolhidasAluno.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao9",flat=True).last()
        resposta10 = questoesEscolhidasAluno.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao10",flat=True).last()
...
def formularioAluno(request,ra):
    turmas = Turmas.objects.raw("SELECT idTurma, ano_letivo, classe, alu_id,prof_id FROM turmas where alu_id=%s GROUP BY classe,ano_letivo",[ra])
    ...
    try:
        ...
        classe = Turmas.objects.filter(alu=pkAluno).values_list("classe",flat=True).last()
        ano_letivo = Turmas.objects.filter(alu=pkAluno).values_list("ano_letivo",flat=True).last()
        fkAluno = Turmas.objects.filter(alu=pkAluno).values_list("alu_id",flat=True)
        if classe == None and ano_letivo == None:
            messages.error(request,"Formulário ainda não foi criado pelo professor")
            return redirect("../telaAluno/"+str(ra))
        questao1 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("questao1",flat=True).last()            
        alternativaAquestao1 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaAquestao1",flat=True).last()
        alternativaBquestao1 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaBquestao1",flat=True).last()
        alternativaCquestao1 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaCquestao1",flat=True).last()
        alternativaDquestao1 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaDquestao1",flat=True).last()
        questao2 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao2",flat=True).last()
        alternativaAquestao2 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaAquestao2",flat=True).last()
        alternativaBquestao2 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaBquestao2",flat=True).last()
        alternativaCquestao2 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaCquestao2",flat=True).last()
        alternativaDquestao2 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaDquestao2",flat=True).last()
        questao3 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao3",flat=True).last()
        alternativaAquestao3 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaAquestao3",flat=True).last()
        alternativaBquestao3 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaBquestao3",flat=True).last()
        alternativaCquestao3 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaCquestao3",flat=True).last()
        alternativaDquestao3 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaDquestao3",flat=True).last()
        questao4 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao4",flat=True).last()
        alternativaAquestao4 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaAquestao4",flat=True).last()
        alternativaBquestao4 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaBquestao4",flat=True).last()
        alternativaCquestao4 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaCquestao4",flat=True).last()
        alternativaDquestao4 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaDquestao4",flat=True).last()
        questao5 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao5",flat=True).last()
        alternativaAquestao5 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaAquestao5",flat=True).last()
        alternativaBquestao5 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaBquestao5",flat=True).last()
        alternativaCquestao5 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaCquestao5",flat=True).last()
        alternativaDquestao5 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaDquestao5",flat=True).last()
        questao6 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao6",flat=True).last()
        alternativaAquestao6 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaAquestao6",flat=True).last()
        alternativaBquestao6 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaBquestao6",flat=True).last()
        alternativaCquestao6 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaCquestao6",flat=True).last()
        alternativaDquestao6 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaDquestao6",flat=True).last()
        questao7 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao7",flat=True).last()
        alternativaAquestao7 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaAquestao7",flat=True).last()
        alternativaBquestao7 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaBquestao7",flat=True).last()
        alternativaCquestao7 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaCquestao7",flat=True).last()
        alternativaDquestao7 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaDquestao7",flat=True).last()
        questao8 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao8",flat=True).last()
        alternativaAquestao8 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaAquestao8",flat=True).last()
        alternativaBquestao8 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaBquestao8",flat=True).last()
        alternativaCquestao8 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaCquestao8",flat=True).last()
        alternativaDquestao8 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaDquestao8",flat=True).last()
        questao9 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao9",flat=True).last()
        alternativaAquestao9 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaAquestao9",flat=True).last()
        alternativaBquestao9 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaBquestao9",flat=True).last()
        alternativaCquestao9 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaCquestao9",flat=True).last()
        alternativaDquestao9 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaDquestao9",flat=True).last()
        questao10 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao10",flat=True).last()
        alternativaAquestao10 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaAquestao10",flat=True).last()
        alternativaBquestao10 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaBquestao10",flat=True).last()
        alternativaCquestao10 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaCquestao10",flat=True).last()
        alternativaDquestao10 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("alternativaDquestao10",flat=True).last()
        if 'atualizar' in request.POST:
            ...
            if classe == None and ano_letivo == None:
                ...
                return redirect("../telaAluno/"+str(ra))
            questao1 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("questao1",flat=True).last()            
            alternativaAquestao1 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaAquestao1",flat=True).last()
            alternativaBquestao1 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaBquestao1",flat=True).last()
            alternativaCquestao1 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaCquestao1",flat=True).last()
            alternativaDquestao1 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaDquestao1",flat=True).last()
            questao2 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao2",flat=True).last()
            alternativaAquestao2 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaAquestao2",flat=True).last()
            alternativaBquestao2 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaBquestao2",flat=True).last()
            alternativaCquestao2 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaCquestao2",flat=True).last()
            alternativaDquestao2 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaDquestao2",flat=True).last()
            questao3 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao3",flat=True).last()
            alternativaAquestao3 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaAquestao3",flat=True).last()
            alternativaBquestao3 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaBquestao3",flat=True).last()
            alternativaCquestao3 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaCquestao3",flat=True).last()
            alternativaDquestao3 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaDquestao3",flat=True).last()
            questao4 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao4",flat=True).last()
            alternativaAquestao4 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaAquestao4",flat=True).last()
            alternativaBquestao4 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaBquestao4",flat=True).last()
            alternativaCquestao4 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaCquestao4",flat=True).last()
            alternativaDquestao4 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaDquestao4",flat=True).last()
            questao5 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao5",flat=True).last()
            alternativaAquestao5 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaAquestao5",flat=True).last()
            alternativaBquestao5 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaBquestao5",flat=True).last()
            alternativaCquestao5 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaCquestao5",flat=True).last()
            alternativaDquestao5 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaDquestao5",flat=True).last()
            questao6 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao6",flat=True).last()
            alternativaAquestao6 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaAquestao6",flat=True).last()
            alternativaBquestao6 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaBquestao6",flat=True).last()
            alternativaCquestao6 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaCquestao6",flat=True).last()
            alternativaDquestao6 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaDquestao6",flat=True).last()
            questao7 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao7",flat=True).last()
            alternativaAquestao7 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaAquestao7",flat=True).last()
            alternativaBquestao7 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaBquestao7",flat=True).last()
            alternativaCquestao7 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaCquestao7",flat=True).last()
            alternativaDquestao7 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaDquestao7",flat=True).last()
            questao8 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao8",flat=True).last()
            alternativaAquestao8 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaAquestao8",flat=True).last()
            alternativaBquestao8 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaBquestao8",flat=True).last()
            alternativaCquestao8 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaCquestao8",flat=True).last()
            alternativaDquestao8 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaDquestao8",flat=True).last()
            questao9 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao9",flat=True).last()
            alternativaAquestao9 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaAquestao9",flat=True).last()
            alternativaBquestao9 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaBquestao9",flat=True).last()
            alternativaCquestao9 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaCquestao9",flat=True).last()
            alternativaDquestao9 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaDquestao9",flat=True).last()
            questao10 = Formulario.objects.filter(ano_letivo=ano_letivo).values_list("questao10",flat=True).last()
            alternativaAquestao10 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaAquestao10",flat=True).last()
            alternativaBquestao10 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaBquestao10",flat=True).last()
            alternativaCquestao10 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaCquestao10",flat=True).last()
            alternativaDquestao10 = Formulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("alternativaDquestao10",flat=True).last()
        if 'responderFomulario' in request.POST:   
            try:
                turma = request.POST.get('turma')
                ano_letivo = turma.split("|")[-2]
                ...
                respostaQuestao9 = request.POST.get("btn-radio9")
                respostaQuestao10 = request.POST.get("btn-radio10")
                questoesEscolhidasAluno.objects.create(classe=classe,ano_letivo=ano_letivo,respostaQuestao1=respostaQuestao1,respostaQuestao2=respostaQuestao2,respostaQuestao3=respostaQuestao3,respostaQuestao4=respostaQuestao4,respostaQuestao5=respostaQuestao5,respostaQuestao6=respostaQuestao6,respostaQuestao7=respostaQuestao7,respostaQuestao8=respostaQuestao8,respostaQuestao9=respostaQuestao9,respostaQuestao10=respostaQuestao10)
                
                #ano_letivo = Turmas.objects.filter(alu=pkAluno).values_list("ano_letivo",flat=True).last()
                resposta1 = RespostasFormulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao1",flat=True).last()
                resposta2 = RespostasFormulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao2",flat=True).last()
                resposta3 = RespostasFormulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao3",flat=True).last()
                resposta4 = RespostasFormulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao4",flat=True).last()
                resposta5 = RespostasFormulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao5",flat=True).last()
                resposta6 = RespostasFormulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao6",flat=True).last()
                resposta7 = RespostasFormulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao7",flat=True).last()
                resposta8 = RespostasFormulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao8",flat=True).last()
                resposta9 = RespostasFormulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao9",flat=True).last()
                resposta10 = RespostasFormulario.objects.filter(classe=classe).filter(ano_letivo=ano_letivo).values_list("respostaQuestao10",flat=True).last()
                acertosAluno=0
                errosAluno=0
                nivelDoAluno = ""
            
                if(resposta1==respostaQuestao1):
                        acertosAluno = acertosAluno+1
                else:
                        errosAluno = errosAluno+1
                if(resposta2==respostaQuestao2):
                        acertosAluno = acertosAluno+1
                else:
                        errosAluno = errosAluno+1
                if(resposta3==respostaQuestao3):
                        acertosAluno = acertosAluno+1
                else:
                        errosAluno = errosAluno+1
                if(resposta4==respostaQuestao4):
                        acertosAluno = acertosAluno+1
                else:
                        errosAluno = errosAluno+1
                if(resposta5==respostaQuestao5):
                        acertosAluno = acertosAluno+1
                else:
                        errosAluno = errosAluno+1
                if(resposta6==respostaQuestao6):
                        acertosAluno = acertosAluno+1
                else:
                        errosAluno = errosAluno+1
                if(resposta7==respostaQuestao7):
                        acertosAluno = acertosAluno+1
                else:
                        errosAluno = errosAluno+1
                if(resposta8==respostaQuestao8):
                        acertosAluno = acertosAluno+1
                else:
                        errosAluno = errosAluno+1
                if(resposta9==respostaQuestao9):
                        acertosAluno = acertosAluno+1
                else:
                        errosAluno = errosAluno+1
                if(resposta10==respostaQuestao10):
                        acertosAluno = acertosAluno+1
                else:
                        errosAluno = errosAluno+1
                ...
                messages.error(request,"Preencha todas as alternativas")
                return redirect("../formularioAluno/"+str(ra))
            except(AttributeError):
                messages.error(request,"Selecione uma classe")
                return redirect("../formularioAluno/"+str(ra))
                
    except(UnboundLocalError):

            messages.error(request,"Formulário não criado")
            return redirect("../telaAluno/"+str(ra))
  


    
    return render(request, "formularioAluno.html", {"classe":classe,"ano_letivo":ano_letivo,"turmas":turmas,"ra":ra,"questao1": questao1,"alternativaAquestao1":alternativaAquestao1, "alternativaBquestao1":alternativaBquestao1,"alternativaCquestao1":alternativaCquestao1,"alternativaDquestao1":alternativaDquestao1,"questao2":questao2,"alternativaAquestao2":alternativaAquestao2, "alternativaBquestao2":alternativaBquestao2,"alternativaCquestao2":alternativaCquestao2,"alternativaDquestao2":alternativaDquestao2,"questao3":questao3,"alternativaAquestao3":alternativaAquestao3, "alternativaBquestao3":alternativaBquestao3,"alternativaCquestao3":alternativaCquestao3,"alternativaDquestao3":alternativaDquestao3,"questao4":questao4,"alternativaAquestao4":alternativaAquestao4, "alternativaBquestao4":alternativaBquestao4,"alternativaCquestao4":alternativaCquestao4,"alternativaDquestao4":alternativaDquestao4,"questao5":questao5,"alternativaAquestao5":alternativaAquestao5, "alternativaBquestao5":alternativaBquestao5,"alternativaCquestao5":alternativaCquestao5,"alternativaDquestao5":alternativaDquestao5,"questao6":questao6,"alternativaAquestao6":alternativaAquestao6, "alternativaBquestao6":alternativaBquestao6,"alternativaCquestao6":alternativaCquestao6,"alternativaDquestao6":alternativaDquestao6,"questao7":questao7,"alternativaAquestao7":alternativaAquestao7, "alternativaBquestao7":alternativaBquestao7,"alternativaCquestao7":alternativaCquestao7,"alternativaDquestao7":alternativaDquestao7,"questao8":questao8,"alternativaAquestao8":alternativaAquestao8, "alternativaBquestao8":alternativaBquestao8,"alternativaCquestao8":alternativaCquestao8,"alternativaDquestao8":alternativaDquestao8,"questao9":questao9,"alternativaAquestao9":alternativaAquestao9, "alternativaBquestao9":alternativaBquestao9,"alternativaCquestao9":alternativaCquestao9,"alternativaDquestao9":alternativaDquestao9,"questao10":questao10,"alternativaAquestao10":alternativaAquestao10, "alternativaBquestao10":alternativaBquestao10,"alternativaCquestao10":alternativaCquestao10,"alternativaDquestao10":alternativaDquestao10})
